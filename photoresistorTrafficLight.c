...
int SMTick2(int state) {
	...
	switch (state) { //Transitions
		...
		case SM2_yellow:									              //Count down for the yellow light
			...
			{
				PORTD = (PORTD & 0xF8)| 0x04;               //Turn off first yellow light and turn on 1st red light
				cnt_SM2 = 0;				
...
int SMTick4(int state) {
	...
	switch (state) { //Transitions
		...
		state = SM4_wait;
		break;
		case SM4_wait:										              //If the car waiting is not on it stays to wait
			if(car_waiting1 == 0)
			{
				state = SM4_wait;
			}
		...
		case SM4_greener:
			...
			else if (cnt_SM4 >= 4)							          //Turn off 2nd green light and turn on second yellow light
			{
